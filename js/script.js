    ...
    if (plugins.preloader.length && !isNoviBuilder) {
      pageTransition({
        ...
        conditions: function (event, link) {
          return (
            !/(\#|callto:|tel:|mailto:|:\/\/)/.test(link) && !event.currentTarget.hasAttribute("data-lightgallery")
          );
        },
    ...
    function initCaptionAnimate(swiper) {
      ...
      if (swiper.params.caption.animationEvent === "slideChangeTransitionEnd") {
        swiper.on(swiper.params.caption.animationEvent, function () {
          initializeAnimation(
            swiper.$wrapperEl[0].children[swiper.previousIndex].querySelectorAll("[data-caption-animate]")
          );
          finalizeAnimation(
            swiper.$wrapperEl[0].children[swiper.activeIndex].querySelectorAll("[data-caption-animate]")
          );
        });
      } else {
